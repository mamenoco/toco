...
// 意味の取りにくい、たとえ話めいた言い回し。
// 授受表現（〜してくれる）は目に見える動きにだけ使い、たとえ話には使いません。
const VAGUE = [
  ['受け皿にな', '「〜に向いています」「〜でも使えます」'],
  ['の受け皿', '「〜に向いています」「〜でも使えます」'],
];
...
function runChecks(article, project, inventory, meta) {
  ...
  // 電球アイコンの帯（結論の1文）の句読点
  // 見出しに近い扱いの1行なので、句読点を入れると読みにくくなります。
  {
    const ls = String(text).split('\n');
    let inProd = false;
    const hits = [];
    ls.forEach((l, i) => {
      if (/^##\s/.test(l)) inProd = /おすすめ.*選/.test(l);
      const m = l.match(/^\*\*([^*]+)\*\*\s*$/);
      if (!m || inProd) return;
      let j = i - 1; while (j >= 0 && !ls[j].trim()) j--;
      if (/^\{\{product:/.test(ls[j] || '')) return;   // 商品の一文キャッチは対象外
      if (/[、。]/.test(m[1])) hits.push(m[1]);
    });
    if (hits.length) {
      add('warn', `電球アイコンの見出しに句読点があります（${hits.length}か所）`,
        `「${hits[0]}」など。見出しに近い扱いの1行なので、句読点は使いません。`
        + '対比で区切りが要るところは「／」にして、それ以外は詰めてください。',
        '太字だけの行（電球アイコンの帯になる結論の1文）から、句読点（、。）を取り除いてください。'
        + '「AならX、BならY」のように対比で区切りが要るところは「、」を「／」に置き換え、'
        + 'それ以外は取り除いて詰めます。行末の「。」も消してください。'
        + '商品カードの直後にある一文キャッチの行と、本文の段落は変更しないでください。');
    }
  }

  // 導入の決まり文句
  // 「うさぎと暮らして{{years}}年になりますが、」は、どの記事にも同じ形で入り、
  // 前後の文とつながらないまま差し込まれやすい書き出しです。
  // 体験がその記事の困りごとに直接つながるなら書き出しを変え、つながらないなら消します。
  {
    const stock = String(text).match(/うさぎと暮らして(?:\{\{years\}\}|[0-9０-９一二三四五六七八九十]+)年になりますが、[^。]*。/g) || [];
    if (stock.length) {
      add('warn', '導入が決まり文句になっています',
        `「${stock[0].slice(0, 40)}…」`
        + ' どの記事にも同じ書き出しが入っていて、前後の文とつながらないまま差し込まれやすい言い方です。'
        + 'その体験が直前の困りごとに直接つながるなら「わが家でも、〜」のように書き出しを変え、'
        + 'つながらないなら文ごと消してください。導入に体験の文はなくても構いません。',
        '導入にある「うさぎと暮らして{{years}}年になりますが、」で始まる文を見直してください。'
        + 'その文が、直前に書いた読者の困りごとに直接つながる体験なら、「わが家でも、〜」のように書き出しを変えて、'
        + '前の文から自然に続く形にしてください。つながらない内容なら、文ごと削除してください。'
        + '削除したことで段落の流れが途切れる場合だけ、前後の文を最小限整えてください。導入以外の箇所は変更しないでください。');
    }
  }
  ...
  {
    let rest = String(text);
    VAGUE.slice().sort((a, b) => b[0].length - a[0].length).forEach(([word, better]) => {
      const n = (rest.match(new RegExp(word, 'g')) || []).length;
      if (!n) return;
      rest = rest.split(word).join('');
      add('warn', `分かりにくい言い回し「${word}」（${n}か所）`,
        `何がどうなるのかが読者に伝わりません。${better} のように、`
        + '中身をそのまま書いてください。（トイレのスノコの下など、実物を指すときは問題ありません）',
        `本文の「${word}」を含む文を、${better} のような具体的な書き方に直してください。`
        + 'たとえ話にせず、何がどうなるのかをそのまま書いてください。ほかの箇所は変更しないでください。');
    });
  }
